...
const root = process.cwd(), stamp = new Date().toISOString().replace(/[-:.]/g, ""),
  folder = fs.mkdtempSync(path.join(root, "docs", ".compare", "domestic-web-live-")), bundle = path.join(folder, "production.mjs");
...
try {
  ...
  for (const [platformId, host] of [["douyin", "creator.douyin.com"], ["kuaishou", "cp.kuaishou.com"], ["xiaohongshu", "creator.xiaohongshu.com"], ["bilibili", "member.bilibili.com"], ["baijiahao", "baijiahao.baidu.com"], ["weixin_channels", "channels.weixin.qq.com"]]) {
  ...
  result.passed = result.targets.length === 6 && result.targets.every((item) => item.permitGranted && item.tlsVerified && item.httpStatus !== null);
  ...
  const output = path.join(root, "docs", ".compare", `domestic-web-live-${stamp}.results.json`);

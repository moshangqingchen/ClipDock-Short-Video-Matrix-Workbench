/** Build only by default. --run-once invokes ONE production anonymous DIRECT/IPIP probe.
...
function tempRoot(value) {
  ...
  assert(path.dirname(resolved) === path.resolve(root, "docs/.compare") && path.basename(resolved).startsWith("domestic-direct-egress-"), "ISOLATION_REQUIRED");
  return resolved;
}
...
async function parent() {
  ...
  const temporary = fs.mkdtempSync(path.join(base, "domestic-direct-egress-"));
  const timestamp = new Date().toISOString().replace(/[-:.]/g, "");
  const reportFile = path.join(root, `docs/domestic-direct-egress.${timestamp}.results.json`);
  ...
  try {
    ...
    const built = await build({
      stdin: { contents: [
        ...
        "export {verifyDomesticTunnelEvidence} from './src/main/api/proxy-tunnel-evidence.ts';",
...
async function child({ app, BrowserWindow, webContents, temporary }) {
  const result = {
    ...
    target: { host: "myip.ipip.net", port: 443 }, controller, proxy,
  ...
  try {
    ...
    reader = new production.ProxyTunnelReader({ targetScope: "domestic", controllerUrl: "http://127.0.0.1:9790", proxy, getSecret: () => null, readVersion: () => generationValid ? version : null });
    const observedReader = {
      readAnonymousTunnel: async (context, signal) => {
        ...
        try {
          ...
          if (evidence) {
            stage.destinationFamily = net.isIP(evidence.connections.value.connections[0]?.metadata?.destinationIP ?? "");
            stage.directChain = JSON.stringify(evidence.connections.value.connections[0]?.chains) === JSON.stringify(["DIRECT"]);
            const verified = production.verifyDomesticTunnelEvidence({ context, evidence, controller, proxy, ...version, nowMono: performance.now(), readStartedAtMono: stage.startedAtMono });
    ...
    probe = new production.ProxyEgressProbe({ domesticDirect: true, proxy, controller, reader: observedReader, readVersion: () => generationValid ? version : null, allowedCountries: ["CN"] });
